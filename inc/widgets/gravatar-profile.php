 ...
 *
 */
add_action( 'widgets_init', 'checkout_gravatar_profile_widget_init' );

function checkout_gravatar_profile_widget_init() {
	// Return early if Jetpack's widget is loaded
	if( class_exists( 'Jetpack_Gravatar_Profile_Widget' ) ) {
		return;
	}

	register_widget( 'Checkout_Gravatar_Profile_Widget' );
}
 ...
 */
class Checkout_Gravatar_Profile_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'grofile',
			__( 'Checkout: Gravatar Profile', 'checkout' ),
			array(
				'classname'   => 'widget-grofile grofile',
				'description' => __( 'Display a mini version of your Gravatar Profile', 'checkout' )
			)
		);
	...
	function widget( $args, $instance ) {
		...
		if ( !$instance['email'] ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '<p>' . sprintf( __( 'You need to select what to show in this <a href="%s">Gravatar Profile widget</a>.', 'checkout' ), admin_url( 'widgets.php' ) ) . '</p>';
		...
		if( ! empty( $profile ) ) {
			...
			<h4><a href="<?php echo esc_url( $profile['profileUrl'] ); ?>" class="grofile-full-link"><?php echo esc_html( apply_filters( 'checkout_gravatar_full_profile_title', __( 'View Full Profile &rarr;', 'checkout' ) ) ); ?></a></h4>
			...
			if ( current_user_can( 'edit_theme_options' ) ) {
				echo '<p>' . esc_html__( 'Error loading profile', 'checkout' ) . '</p>';
			}
		}
	...
	function display_personal_links( $personal_links = array() ) {
		...
		?>
			...
			<h4><?php echo esc_html( apply_filters( 'checkout_gravatar_personal_links_title', __( 'Personal Links', 'checkout' ) ) ); ?></h4>
	...
	function display_accounts( $accounts = array() ) {
		...
		<h4><?php echo esc_html( apply_filters( 'checkout_gravatar_verified_services_title', __( 'Verified Services', 'checkout' ) ) ); ?></h4>
		...
		<?php foreach( $accounts as $account ) :
			...
			<li>
				<a href="<?php echo esc_url( $account['url'] ); ?>" title="<?php echo sprintf( _x( '%1$s on %2$s', '1: User Name, 2: Service Name (Facebook, Twitter, ...)', 'checkout' ), esc_html( $account['display'] ), esc_html( $sanitized_service_name ) ); ?>">
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'email_user' ); ?>">
				<?php esc_html_e( 'Select a user or pick "custom" and enter a custom email address.', 'checkout' ); ?>
				<br />

				<?php wp_dropdown_users( array(
					'show_option_none' => __( 'Custom', 'checkout' ),
					'selected'         => $email_user,
		...
		<p class="gprofile-email-container <?php echo empty( $email_user ) || $email_user == -1 ? '' : 'hidden'; ?>">
			<label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php esc_html_e( 'Custom Email Address', 'checkout' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_personal_links' ); ?>">
				...
				<?php esc_html_e( 'Show Personal Links', 'checkout' ); ?>
				<br />
				<small><?php esc_html_e( 'Links to your websites, blogs, or any other sites that help describe who you are.', 'checkout' ); ?></small>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'show_account_links' ); ?>">
				...
				<?php esc_html_e( 'Show Account Links', 'checkout' ); ?>
				<br />
				<small><?php esc_html_e( 'Links to services that you use across the web.', 'checkout' ); ?></small>
			</label>
		</p>

		<p><a href="<?php echo esc_url( $profile_url ); ?>" target="_blank" title="<?php esc_attr_e( 'Opens in new window', 'checkout' ); ?>"><?php esc_html_e( 'Edit Your Profile', 'checkout' )?></a> | <a href="http://gravatar.com" target="_blank" title="<?php esc_attr_e( 'Opens in new window', 'checkout' ); ?>"><?php esc_html_e( "What's a Gravatar?", 'checkout' ); ?></a></p>
